...
void load_changes( void )
{
   ...
   {
      bug( "Could not open changes.dat file for reading." );
      return;
   }
...
void do_addchange( CHAR_DATA * ch, const char *argument )
{
   ...
   if( argument[0] == '\0' )
   {
      ...
      send_to_char( "&wTypes are: code, area, help, rule, typo.\n\r", ch );
      send_to_char( "&wType '&Rchanges&w' to view the list.\n\r", ch );
      return;
   }
   ...
   sprintf( buf, "&z<&CCHANGE&z> &w%s change &R#%d &wadded, type '&Rchanges&w' to see the details.&w\n\r",
...
void do_delchange( CHAR_DATA * ch, const char *argument )
{
   char arg1[MAX_INPUT_LENGTH];
   char buf[MSL];
   int num;
   ...
   {
      send_to_char( "&wFor delchange you must provide a change number.\n\r", ch );
...
void do_changes( CHAR_DATA * ch, const char *argument )
{
   ...
   if( maxChanges < 1 )
   {
      send_to_char( "Max changes less than one\r\n", ch );
      return;
   }
   ...
   send_to_char( "\n\r", ch );
   send_to_pager( "&z-=[ War &rfor &RMidgaard &WChangelog &z]=-\r\n", ch );
   send_to_pager( "&z--------------------------------------------------------------------------------&w\n\r", ch );
   send_to_pager( "&RNo.  Coder        Date        Type    Change&w\n\r", ch );
   send_to_pager( "&z--------------------------------------------------------------------------------&w\n\r", ch );
   ...
   if( is_number( arg ) )
   {
      ...
      {
         pager_printf( ch, "&z[&R%3d&z] &W%-11s &C*%-6s &R%-5s &P%-45s&z\n\r", ( i + 1 ), changes_table[i].coder,
                    changes_table[i].date, changes_table[i].type, change_justify( (char *)changes_table[i].change, 45 ) );
      }
   }
   else {
   ...
   for( i = 0; i < maxChanges; i++ )
   {
      ...
      pager_printf( ch, "&z[&R%3d&z] &W%-11s &C%-6s &R%-5s &P%-55s&z\n\r", ( i + 1 ), changes_table[i].coder,
               changes_table[i].date, changes_table[i].type, change_justify( (char *)changes_table[i].change, 55 ) );
   }
   }
   send_to_pager( "&z--------------------------------------------------------------------------------&w\n\r", ch );
   pager_printf( ch, "&R    There are &z[ &Y%d&z ] &Rchanges in the database, &Y%d &Rof them were added today.&w\n\r",
            maxChanges, today );
   send_to_pager( "&z       Also see: '&Cchanges all&z' for a list of all the changes.&w\n\r", ch );
   send_to_pager( "&z             Or: '&Cchanges <type>&z' for a filtered list of all.&w\n\r", ch );
   if( totalpages > 1 )
   {
      pager_printf( ch, "&z             Or: '&Cchanges &c<&Y1 &Cto&Y %d&c>&z' for individual pages.&w\n\r", totalpages );
   }
   send_to_pager( "&z--------------------------------------------------------------------------------&w\n\r", ch );
   return;
}
