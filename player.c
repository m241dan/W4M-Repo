...
void do_worth( CHAR_DATA* ch, const char* argument)
{
   ...
   pager_printf( ch, "|Level: %-4d |Favor: %-10s |Alignment: %-9s |Experience: %-9ld|\r\n", ch->level, buf, buf2, ch->experience[ch->Class] );
   send_to_pager( " ----------------------------------------------------------------------------\r\n", ch );
   switch ( ch->style )
   {
      case STYLE_EVASIVE:
...
void do_score( CHAR_DATA* ch, const char* argument )
{
   ...
   */
   ...
   pager_printf( ch, "XP   : %-9ld        Mana: %-5d of %5d   MKills:  %-5.5d    AutoLoot(%c)\r\n",
                 ch->experience[ch->Class], ch->mana, ch->max_mana, ch->pcdata->mkills, xIS_SET( ch->act, PLR_AUTOLOOT ) ? 'X' : ' ' );
...
void do_level( CHAR_DATA* ch, const char* argument)
{
   ...
   ch_printf( ch, "\r\nExperience required, levels %d to %d:\r\n______________________________________________\r\n\r\n",
              lowlvl, hilvl );
   snprintf( buf, MAX_STRING_LENGTH, " exp  (Current: %12s)", num_punct( ch->experience[ch->Class] ) );
   snprintf( buf2, MAX_STRING_LENGTH, " exp  (Needed:  %12s)", num_punct( exp_level( ch, ch->level + 1 ) - ch->experience[ch->Class] ) );
...
void do_statreport( CHAR_DATA* ch, const char* argument)
{
   ...
   snprintf( buf, MAX_STRING_LENGTH, "$n reports: %d/%d hp %d/%d mana %d/%d mv %ld xp.",
             ch->hit, ch->max_hit, ch->mana, ch->max_mana, ch->move, ch->max_move, ch->experience[ch->Class] );
...
void do_report( CHAR_DATA* ch, const char* argument)
{
   ...
   snprintf( buf, MAX_INPUT_LENGTH, "$n reports: %d/%d hp %d/%d mana %d/%d mv %ld xp.",
             ch->hit, ch->max_hit, ch->mana, ch->max_mana, ch->move, ch->max_move, ch->experience[ch->Class] );
