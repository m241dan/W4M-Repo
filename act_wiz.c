...
void do_ostat( CHAR_DATA* ch, const char* argument)
{
   ...
   ch_printf_color( ch, "&cIndex Range: &w%-2d  &cIndex DamType: &w%s\r\n", obj->pIndexData->range, damage_type_name( &obj->pIndexData->damtype ) );
   ch_printf_color( ch, "&cObject Range: &w%-2d &cObject DamType: &w%s\r\n", obj->range, damage_type_name( &obj->damtype ) );
...
void do_mstat( CHAR_DATA* ch, const char* argument)
{
   ...
   {
      pager_printf_color( ch, "&cRange   : &w%d\r\n", victim->range );
   ...
   pager_printf_color( ch, "&cSpeaks: &w%d   &cSpeaking: &w%d   &cExperience: &w%ld",
                       victim->speaks, victim->speaking, victim->experience[victim->Class] );
...
void do_balzhur( CHAR_DATA* ch, const char* argument)
{
   ...
   victim->experience[victim->Class] = 2000;
   victim->max_hit = 10;
   victim->max_mana = 100;
   victim->max_move = 100;
...
void do_mortalize( CHAR_DATA* ch, const char* argument)
{
   ...
   if( stat( fname, &fst ) != -1 )
   {
      ...
      victim->experience[victim->Class] = exp_level( victim, LEVEL_AVATAR );
      victim->max_hit = 800;
      victim->max_mana = 800;
      victim->max_move = 800;
      for( sn = 0; sn < num_skills; ++sn )
...
void do_project( CHAR_DATA* ch, const char* argument)
{
   ...
   if( !str_cmp( arg1, "description" ) )
   {
      ...
      start_editing( ch, (char *)pproject->description );
      return;
   }
