...
void load_mobiles( AREA_DATA * tarea, FILE * fp )
{
   ...
   for( ;; )
   {
      ...
      if( letter == 'C' )  /* Realms complex mob   -Thoric */
      {
         ...
         pMobIndex->range = fread_number( fp );
...
CHAR_DATA *create_mobile( MOB_INDEX_DATA * pMobIndex )
{
   ...
   mob->experience[0] = pMobIndex->experience;
...
MOB_INDEX_DATA *make_mobile( int vnum, int cvnum, const char *name )
{
   ...
   else
   {
      ...
      pMobIndex->experience = cMobIndex->experience;
...
void fread_fuss_object( FILE * fp, AREA_DATA * tarea )
{
   ...
   for( ;; )
   {
      ...
      switch ( word[0] )
      {
         ...
         case 'R':
            KEY( "Range", pObjIndex->range, fread_number( fp ) );
            break;
         case 'S':
   ...
   for( ;; )
   {
      ...
      switch ( word[0] )
      {
         ...
         case 'R':
            ...
            KEY( "Range", pMobIndex->range, fread_number( fp ) );
